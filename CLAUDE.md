...
- **#96 CLOSED (night):** PR-C (#114) re-keyed the primary-DB user tables to numeric v18 word ids (progress/statistics/srs_schedule BIGINT; SRS wire contract numeric with [1, 999,999] bounds — custom-word ids ≥1M are per-device and deferred; optional lemma/languageCode hints; SyncService per-item failure for legacy payloads). Suite **248** green. **#113** made the compose deployment actually bootable: prod env names aligned to DB_PRIMARY_*/DB_SECONDARY_* (fail-fast passwords, jdbc-url keys) and the backend image now builds from the monorepo root so the Docker-built jar embeds `seed_v18.json` (verified inside the image; root `.dockerignore` allowlist; compose ×2 + CI docker job updated together).
