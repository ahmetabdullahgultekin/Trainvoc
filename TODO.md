...
- [x] **#192 — No Google Sign-In button** · `LoginScreen.kt`. **DONE 2026-06-05**: Firebase Google credential sign-in wired end-to-end (repo→VM→LoginScreen button via GoogleSignIn + ActivityResultLauncher); clean-checkout-safe web-client-id lookup.
- [x] **#191 — No email-verification UI** · `AuthRepository.kt:219` code path now has a UI. **DONE 2026-06-05**: `EmailVerificationBanner` (resend action) on ProfileScreen for unverified users.
- [x] **#193 — No session-timeout handling** · `AuthRepository.kt`. **DONE 2026-06-05**: `validateSession()` (token force-refresh → sign-out on failure) + `SessionExpiredHandler` dialog routing to login.
- [x] **#194 — Leaderboard is a "Coming Soon" placeholder** · `LeaderboardScreen.kt`. **DONE 2026-06-05**: global board honestly "coming soon" + real local "Your Progress" stats panel for standalone v1.
...
| 191 | Android | `AuthRepository.kt:219` | Email verification code exists but no UI screen to prompt verification | ✅ FIXED 2026-06-05 (dev/2026-06-05): new `EmailVerificationBanner` composable (auth pkg) shows for signed-in, unverified users with a "resend" action; wired into ProfileScreen; `AuthViewModel.sendEmailVerification()/emailVerificationSent`. EN+TR strings. AuthViewModelTest green. |
| 192 | Android | `LoginScreen.kt` | No Google Sign-In option - only email/password despite GoogleAuthManager existing | ✅ FIXED 2026-06-05 (dev/2026-06-05): `FirebaseAuthRepository.signInWithGoogle(idToken)` + `AuthRepository.loginWithGoogle` + `AuthViewModel.loginWithGoogle`; LoginScreen now has a "Sign in with Google" button using GoogleSignIn + ActivityResultLauncher (web client id resolved at runtime from `default_web_client_id`, clean-checkout-safe). Note: the pre-existing GoogleAuthManager is Drive-backup-only, not generic auth. EN+TR strings. AuthViewModelTest green. |
| 193 | Android | `AuthRepository.kt` | No session timeout handling - no automatic logout on token expiration | ✅ FIXED 2026-06-05 (dev/2026-06-05): `AuthRepository.validateSession()` force-refreshes the Firebase ID token; on failure it signs out + drops to NotAuthenticated. `AuthViewModel.validateSession()/sessionExpired`; new `SessionExpiredHandler` composable shows a blocking dialog routing to login (wired in ProfileScreen, validated on screen enter). EN+TR strings. AuthViewModelTest green. |
| 194 | Android | `LeaderboardScreen.kt` | Leaderboard is placeholder "Coming Soon" - not functional | ✅ FIXED 2026-06-05 (dev/2026-06-05): global board kept honestly "coming soon" (needs backend) but the screen now shows the user's real local "Your Progress" panel (current/longest streak, active days, achievements) from new `LeaderboardViewModel` over `GamificationManager.getStats()`. Removed dead `LeaderboardItem`. EN+TR strings. |
...
| 222 | Android | `app/src/test/.../viewmodel/*Test.kt`, `domain/usecase/*Test.kt` | Unit-test source set does NOT compile: ~11 test files use `org.mockito.kotlin` symbols (`whenever`/`any`/`eq`/`verify`/`never`) but **mockito-kotlin is not a declared test dependency** (project uses MockK); some also pass wrong ctor args (e.g. `WordViewModelTest`/`WordOfDayViewModelTest`). `./gradlew :app:testDebugUnitTest` fails at `compileDebugUnitTestKotlin` → no unit tests can run. Found 2026-06-05 while verifying #168's test. | ✅ FIXED 2026-06-05 (dev/2026-06-05): declared `mockito-kotlin 5.4.0` test dep (alongside MockK); fixed stale ctor args/enum drift in 9 test files (`WordViewModel`/`WordOfDay`/`Quiz`/`Settings`/`Multiplayer`/`Favorites`/`CloudBackup`/`SubmitQuizAnswer`/`Gamification`) against current models (`WordLevel` enum, `Statistic`, `QuizType.enums`, `DriveBackup*`, `AchievementProgress`, `RoomSettings`/`PlayerRanking`/`RoomListItem`, `gamificationManager` ctor param). `compileDebugUnitTestKotlin` BUILD SUCCESSFUL; `:app:testDebugUnitTest` now runs 172 tests (was 0 — wouldn't compile). Also fixed the pre-existing `TestDispatcherProvider` "different schedulers" defect (new `MainDispatcherRule` + delegate to `Dispatchers.Main`) and added `testOptions.unitTests.isReturnDefaultValues=true` → pass rate 0→142/172. Remaining 30 failures are pre-existing Android-SDK-static-init / mock-setup issues (CloudBackup→GoogleAuthManager, Stats→DateFormat+UnfinishedStubbing) tracked as #223. |
| 223 | Android | `CloudBackupViewModelTest.kt`, `StatsViewModelTest.kt`, + ~6 others | 30 unit tests fail at RUNTIME (suite now compiles+runs after #222). Causes are pre-existing: (a) `CloudBackupViewModelTest` (16) → `NoClassDefFoundError` initializing `GoogleAuthManager` (Play-Services static init unavailable on plain JVM — needs Robolectric `@RunWith(RobolectricTestRunner)` or a fakeable auth seam); (b) `StatsViewModelTest` (9) → `android.icu.text.DateFormat` not mocked (Robolectric needed) + an `UnfinishedStubbingException` (real mock-setup bug at `setupOtherAnalyticsMocks`); (c) stragglers in `GetUserStats`/`WordOfDay`/`Favorites`/`Settings` (mock returns Error→cast fails / un-stubbed flow / assertion mismatch). *Done when*: failing classes either run under Robolectric or have their Android-SDK seams faked, and the genuine mock bugs are corrected. | ⬜ OPEN |
