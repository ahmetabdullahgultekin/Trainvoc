...
fun LoginScreen(
    ...
    // Google Sign-In (#192). The web client id is generated by the google-services
    // plugin into `default_web_client_id` when google-services.json is present.
    // We resolve it at runtime so a clean checkout (no Firebase file) still compiles;
    // when the id is absent we fall back to a basic (email-only) sign-in request and
    ...
    val googleSignInLauncher = rememberLauncherForActivityResult(
        ...
        if (result.resultCode == Activity.RESULT_OK) {
            ...
            // User cancelled — no error toast needed, just ignore.
        }
    }
