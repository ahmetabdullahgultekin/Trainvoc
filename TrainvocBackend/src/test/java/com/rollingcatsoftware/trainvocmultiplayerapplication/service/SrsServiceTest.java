...
class SrsServiceTest {
    ...
    private SrsReviewItem item(String wordId, long dueAt, String state, long clientUpdatedAt) {
        return new SrsReviewItem(wordId, dueAt, 8.43, 0.27, dueAt - 86_400_000L, state, clientUpdatedAt);
    }
    ...
    class UpsertReviews {
        ...
        void insertsNewRow() {
            when(repository.findByUserIdAndWordId(USER, "abandon")).thenReturn(Optional.empty());

            SrsReviewBatchRequest req = new SrsReviewBatchRequest(
                    List.of(item("abandon", 1_749_340_800_000L, "REVIEW", 1_749_254_400_000L)));
            ...
            assertEquals("abandon", row.getWordId());
        ...
        void updatesWhenNewer() {
            SrsSchedule existing = new SrsSchedule("abandon", USER);
            ...
            when(repository.findByUserIdAndWordId(USER, "abandon")).thenReturn(Optional.of(existing));

            SrsReviewBatchRequest req = new SrsReviewBatchRequest(
                    List.of(item("abandon", 2_000L, "REVIEW", 2_000L)));
        ...
        void skipsStaleWrite() {
            ...
            SrsReviewBatchRequest req = new SrsReviewBatchRequest(
                    List.of(item("abandon", 1_000L, "LEARNING", 1_000L)));
        ...
        void equalTimestampIsStale() {
            ...
            SrsReviewBatchRequest req = new SrsReviewBatchRequest(
                    List.of(item("abandon", 9_000L, "RELEARNING", 3_000L)));
        ...
        void mixedBatch() {
            ...
            when(repository.findByUserIdAndWordId(USER, "alpha")).thenReturn(Optional.empty());
            // second word: stale -> skipped
            SrsSchedule beta = new SrsSchedule("beta", USER);
            beta.setClientUpdatedAt(Instant.ofEpochMilli(10_000L));
            beta.setDueAt(Instant.ofEpochMilli(10_000L));
            when(repository.findByUserIdAndWordId(USER, "beta")).thenReturn(Optional.of(beta));

            SrsReviewBatchRequest req = new SrsReviewBatchRequest(List.of(
                    item("alpha", 1_000L, "NEW", 1_000L),
                    item("beta", 2_000L, "REVIEW", 2_000L)));
        ...
        void nullLastReviewed() {
            when(repository.findByUserIdAndWordId(USER, "new-word")).thenReturn(Optional.empty());
            SrsReviewItem withNull = new SrsReviewItem(
                    "new-word", 1_000L, 1.0, 0.3, null, "NEW", 1_000L);
    ...
    class GetSchedule {
        ...
        void returnsScheduleWithAggregates() {
            SrsSchedule a = new SrsSchedule("abandon", USER);
            ...
            assertEquals("abandon", mapped.wordId());
