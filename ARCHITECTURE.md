...
### Android (Room SQLite) — relational multilingual schema (v18)

Since schema v18 the vocabulary is **relational and multilingual**: every
word in every language is a `words` row with a permanent numeric id
(seed ids are pinned by `tools/dictgen/ids.lock.json`; user-added words
get ids ≥ 1,000,000). Meanings are cross-lingual many-to-many edges
grouped into senses; synonyms are same-language id pairs. Adding a
language (e.g. Arabic) is data-only — insert a `languages` row plus word
and translation rows; no schema change.
...
├──────────────────┤      ├──────────────────────────────┤
│ id (PK)          │←─────│ id (PK, numeric)             │
│ code ("en","tr") │      │ word (lemma)                 │
│ name             │      │ language_id (FK)             │
└──────────────────┘      │ meaning (display cache)      │
                          │ level (CEFR) / note / POS    │
                          │ SM-2 + favorites + stat_id   │
                          │ UNIQUE(word, language_id)    │
                          └──────────────────────────────┘
                             │            │           │
              ┌──────────────┘            │           └────────────┐
              ▼ M:N (cross-lingual)       ▼ M:N (same-language)    ▼ M:N
┌───────────────────────────┐  ┌────────────────────┐  ┌────────────────────┐
│     word_translations     │  │      synonyms      │  │ word_exam_cross_ref│
├───────────────────────────┤  ├────────────────────┤  ├────────────────────┤
│ word_id (FK, CASCADE)     │  │ word_id (FK)       │  │ word_id (FK)       │
│ translated_word_id (FK)   │  │ synonym_word_id(FK)│  │ exam (FK)          │
│ sense_index               │  └────────────────────┘  └────────────────────┘
│ note / is_primary         │   "take" → sense 0: almak; sense 1: götürmek…
└───────────────────────────┘
```

`word_of_day` and `quiz_question_results` reference `words.id` with real
FOREIGN KEYs (CASCADE). The seed database and manifest
(`assets/database/trainvoc-db.db`, `seed_v18.json`) are generated by
`tools/dictgen` from Room's exported schema DDL, so `createFromAsset`
validation can never drift. The Room migration 17→18 rebuilds the word
tables from the manifest and carries all user progress over by lemma.
